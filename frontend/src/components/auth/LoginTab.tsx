...
const LoginTab: React.FC<LoginTabProps> = ({ onSuccess }) => {
  ...
  const [takeoverSessionInfo, setTakeoverSessionInfo] = useState<SessionInfo | null>(null);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const result = await login(normalizedEmail, loginData.password, loginData.role);

      if (result.requiresSessionTakeover && result.sessionInfo) {
        setTakeoverSessionInfo(result.sessionInfo);
  ...
  const handleTakeoverConfirm = async () => {
    ...
    try {
      // Use the AuthContext function which explicitly calls /takeover-session
      ...
      if (result.user) {
        toast({
          ...
          description: 'Previous session logged out successfully.',
        });
      ...
      toast({
        ...
        description: error.message || 'Could not takeover session. Please try again.',
        variant: 'destructive',
      });
      setShowTakeoverModal(false); // Close modal on failure so user can retry
    } finally {
  ...
  const handleTakeoverCancel = () => {
    setShowTakeoverModal(false);
    setTakeoverSessionInfo(null);
    // login Data remains in form, user is not logged in.
  };

  return (
    <>
      {showTakeoverModal && takeoverSessionInfo && (
        <SessionTakeoverModal
          sessionInfo={takeoverSessionInfo}
